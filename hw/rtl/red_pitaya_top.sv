...
localparam NMACS  = 16;
localparam NSPINS = 128;
 im_core
   #(.NMACS(NMACS),                 // Must be a factor of NSPINS
     .NSPINS(NSPINS),               // NSPINS/NMACS = integer
     .AWIDTH(2 * $clog2(NSPINS)),   // These many address lines
     .DWIDTH(14)                    // Limited by DAC or MAC
    ) i_im_core
   (.clk(adc_clk),
    .rst_n(adc_rstn),
    .sys_addr(sys[3].addr),
